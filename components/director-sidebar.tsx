...
export default function DirectorSidebar({ profile, company }: DirectorSidebarProps) {
  ...
  const navItems = [
    ...
    { href: "/dashboard/director/movements", label: "MOVIMIENTOS", icon: TrendingUp },
    { href: "/dashboard/director/settings", label: "CONFIGURACIÓN", icon: Settings },
  ]
    ...
    <aside className="fixed left-0 top-0 h-screen w-[280px] bg-[#0d2646] text-white p-6 flex flex-col">
      ...
      <Link href="/dashboard/profile" className="mb-8">
        <div className="flex flex-col items-center gap-3 p-4 rounded-lg hover:bg-white/10 transition-colors">
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-gray-300">{profile?.rol || "Director General"}</p>
          </div>
        </div>
      </Link>
      ...
      <nav className="flex-1 space-y-2">
        {navItems.map((item) => {
          ...
          const isActive =
            pathname === item.href ||
            (item.href === "/dashboard/director" && pathname?.startsWith("/dashboard/director/product"))
          return (
            <Link key={item.href} href={item.href}>
              <Button
                variant={isActive ? "secondary" : "ghost"}
                className={`w-full justify-start text-left gap-3 ${
                  isActive ? "bg-white text-[#0d2646] hover:bg-white/90" : "text-white hover:bg-white/10"
                }`}
              >
