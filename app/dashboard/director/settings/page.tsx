...
export default async function DirectorSettingsPage() {
  ...
  const { data: abcCategories } = await supabase
    .from("abc_categories")
    .select("*")
    .eq("company_id", profile.company_id);
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Ubicaciones de tu empresa ({warehouses?.length || 0})
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            {warehouses?.map((warehouse) => (
              ...
              >
                ...
                <span className="text-sm text-gray-600">
                  Capacidad: {warehouse.capacidad_maxima}
                </span>
              </div>
            ))}
          </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configuración de categorías de rotación
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-2">
